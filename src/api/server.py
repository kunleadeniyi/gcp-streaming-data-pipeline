...
def publish_message():
    ...
    try:
        ...
        # body['data']['receive_timestamp'] = str(datetime.datetime.now()) # changing this to integer in microseconds format
        # gcp big query requires it to be in INT64 microseconds format to convert to timestamp
        body['data']['receive_timestamp'] = int(datetime.datetime.now().timestamp() * 1e6)
        ...
        valid_topic_path = publisher.topic_path(project_id, f"{body['table_name']}_topic")
        print(valid_topic_path)
        future = publisher.publish(valid_topic_path, data=json.dumps(body['data']).encode())
